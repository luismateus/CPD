...
void massCenter_each_cell(int npar, int ncell, particle_t *par, cell_t *cell) {
    int n, i;
    cell_t *cell2 = (cell_t *)malloc(ncell * ncell * sizeof(cell_t));
    double cellX[ncell*ncell],cellY[ncell*ncell],cellM[ncell*ncell];

    #pragma omp parallel 
    {
        #pragma omp for reduction(+:cellX,cellY,cellM), private(n)
            for (i = 0; i < npar; i++) {
                n=par[i].c;
                if (!cellM[n]){
    ...
    for (n = 0; n < pow(ncell,2); n++) {
        if(cellM[n]!=0){
            cell[n].m = cellM[n];
            cell[n].x = cellX[n]/cellM[n];
            cell[n].y = cellY[n]/cellM[n];
        }
